...
async function loadBalance() {
  try {
    const res = await fetch(`${API}/api/wallet/balance`, { headers: { Authorization: `Bearer ${getToken()}` }});
...
async function fundWallet() {
  ...
  try {
    ...
    if (res.ok) showToast("Wallet funded!", "success"), setTimeout(() => window.location = "dashboard.html", 1500);
    else { const msg = await res.text(); showToast(msg || "Funding failed", "error"); }
  } catch { showToast("Funding failed", "error"); }
}
...
async function loadTransactions() {
  ...
  if (!list) return;
  try {
    const res = await fetch(`${API}/api/wallet/transactions`, { headers: { Authorization: `Bearer ${getToken()}` }});
    const data = await res.json();
    ...
    list.innerHTML = data.map(tx => {
      ...
      const sign = isCredit ? "+" : "−";
      const icon = isCredit
        ? `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#00e5a0" stroke-width="2" stroke-linecap="round">
             <polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/><polyline points="17 6 23 6 23 12"/></svg>`
        : `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#ff4d6a" stroke-width="2" stroke-linecap="round">
...
function openPinModal() {
  const toEmail = getInput("toEmail"), amount = getInput("amount");
  if (!toEmail || !amount) return showToast("Fill in all fields", "error");
  document.getElementById("pinModal").classList.add("open");
}
function closeModal() { document.getElementById("pinModal").classList.remove("open"); }

async function confirmSend() {
  const toEmail = getInput("toEmail"), amount = getInput("amount"), pin = getInput("pinInput");
  ...
  try {
    ...
    if (res.ok) showToast("Transfer successful!", "success"), closeModal(), setTimeout(() => window.location = "dashboard.html", 1500);
    else showToast(msg || "Transfer failed", "error");
...
// ── Navigation ──
const nav = {
  send: () => window.location = "send.html",
  fund: () => window.location = "fund.html",
  transactions: () => window.location = "transaction.html",
  logout: () => { localStorage.removeItem("token"); window.location = "index.html"; }
};

// ── Auto-load per page ──
const path = window.location.pathname;
if (path.includes("dashboard")) { loadBalance(); loadAvatar(); }
